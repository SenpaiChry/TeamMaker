...
/**
 * Test per MatchUtility.BY_DAY_TIME: prima per day, poi per orario (in MINUTI:
 * il confronto stringa sbagliava "9:30" vs "10:00"). Anche i comportamenti su
 * orari malformati (via TimeUtility.toMinutes) sono verificati qui perche' il
...
public class MatchByDayTimeTest {
    ...
    public void sameDay_sortedByMinutes_notByLexicographic() {
        ...
        Collections.sort(list, MatchUtility.BY_DAY_TIME);
    ...
    public void sameDayAndTime_returnsZero() {
        ...
        assertEquals(0, MatchUtility.BY_DAY_TIME.compare(a, b));
    }

    @Test
    public void malformedTime_treatedAsZeroMinutes() {
        // TimeUtility.toMinutes(malformed) -> 0, quindi la partita finisce all'inizio.
    ...
    public void bigList_sortedStableChronologically() {
        ...
        Collections.sort(shuffled, MatchUtility.BY_DAY_TIME);
        assertEquals(Arrays.asList(m1, m2, m3, m4, m5), shuffled);
    }

    @Test
    public void matchCompareToDelegatesToBYDAYTIME() {
        // Match implements Comparable via MatchUtility.BY_DAY_TIME: verifichiamo la delega.
