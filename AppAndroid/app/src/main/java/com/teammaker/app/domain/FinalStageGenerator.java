...
public class FinalStageGenerator {
     ...
     * WINNER/LOSER puntano alle partite corrette; vanno salvate con MatchRepository.saveMatches.
    ...
    public static List<Match> generate(Tournament tournament, int bracketSize, boolean thirdPlace) {
        ...
        if (thirdPlace) {
            ...
            if (semis != null) {
                ...
                m = new Match("TO DO", "TO DO", 0, "0:00", 0, 0, MatchPhases.THIRD);
    ...
    private static String labelForCount(int matchCount) {
        if (matchCount >= 4) return MatchPhases.QUARTER;
        if (matchCount == 2) return MatchPhases.SEMIFINAL;
