...
public class MatchScheduler {
    ...
    public static void generateItalianBracket(String tournamentKey, ArrayList<String> stringsInput, String minutesPerMatch, boolean homeAndAway) {
        ...
        List<List<Match>> rounds = buildRoundRobinRounds(tournament.teams, MatchPhases.GROUP);
        if (homeAndAway) {
        ...
        MatchRepository.saveMatches(tournament.key, matches);

        tournament.matches = matches;
        TeamRepository.saveBracketForTeams(tournamentKey);
        TournamentRepository.updateNBracketsTournament(tournamentKey, 1);
    }
    ...
    public static void generateMultipleBracket(String tournamentKey, ArrayList<String> stringsInput, String minutesPerMatch, int nBrackets, boolean homeAndAway) {
        ...
        for (int b = 0; b < nBrackets; b++) {
            ...
            List<List<Match>> rounds = buildRoundRobinRounds(teams, MatchPhases.groupCode(getBracketLetter(b)));
            if (homeAndAway) {
                rounds.addAll(mirrorRounds(rounds));
            }
        ...
        MatchRepository.saveMatches(tournament.key, globalOrder);

        tournament.matches = new ArrayList<>(globalOrder);
        TeamRepository.saveBracketForTeams(tournamentKey);
        TournamentRepository.updateNBracketsTournament(tournamentKey, nBrackets);
    }
    ...
    private static List<List<Match>> buildChampionsRounds(List<Team> originalTeams, int k) {
        ...
        for (int d = 1; d <= k / 2; d++) {
            ...
            for (int i = 0; i < n; i++) {
                ...
                round.add(new Match(t1.key, t2.key, 0, "0:00", 0, 0, MatchPhases.GROUP));
            }
            rounds.add(round);
        }
