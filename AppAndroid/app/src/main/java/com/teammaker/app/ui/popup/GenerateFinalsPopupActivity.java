...
import com.teammaker.app.domain.FinalStageGenerator;
...
public class GenerateFinalsPopupActivity extends AppCompatActivity {
    ...
    private void updatePreview() {
        ...
        int qi = 1, si = 1;
        for (Match m : matches) {
            if (m.type.equals(MatchPhases.QUARTER)) code.put(m.key, "Q" + (qi++));
            else if (m.type.equals(MatchPhases.SEMIFINAL)) code.put(m.key, "S" + (si++));
            else if (m.type.equals(MatchPhases.FINAL)) code.put(m.key, "F");
    ...
    private void confirm() {
        ...
        if (!Times.checkInputTime(slots, minutes, matches.size())) {
        ...
        int n = matches.size();
        if (n >= 2 && matches.get(n - 1).type.equals(MatchPhases.THIRD) && matches.get(n - 2).type.equals(MatchPhases.FINAL)) {
            Collections.swap(matches, n - 1, n - 2);
        }

        MatchScheduler.assignDayAndTime(matches, slots, minutes);
        MatchRepository.saveMatches(tournamentKey, matches);
