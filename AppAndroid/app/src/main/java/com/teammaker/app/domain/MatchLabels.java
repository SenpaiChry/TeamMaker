...
public class MatchLabels {
    ...
    private static String codeOf(Context ctx, Tournament tournament, String matchKey) {
        ...
        for (Match m : tournament.matches) {
            ...
            if (m.type.equals(MatchPhases.QUARTER)) quarters.add(m);
            else if (m.type.equals(MatchPhases.SEMIFINAL)) semis.add(m);
            else if (m.type.equals(MatchPhases.FINAL)) finalMatch = m;
        }
