...
import com.teammaker.app.domain.ScheduleOptions;
import com.teammaker.app.domain.Times;
import com.teammaker.app.domain.MatchScheduler;
import com.teammaker.app.data.repository.TournamentRepository;

import java.util.ArrayList;
import com.teammaker.app.ui.common.MaxHeightScrollView;
import com.teammaker.app.R;

public class GenerateBracketPopupActivity extends AppCompatActivity {
    ...
    private int computeMatches() {
        int nTeams = TournamentRepository.getTournamentByKey(tournamentKey).teams.size();
        ...
        switch (format) {
            case GROUPS: {
                ...
                return MatchScheduler.nMatchesMultipleBracket(tournamentKey, nBrackets) * (homeAndAway ? 2 : 1);
            }
            case CHAMPIONS: {
                int k = parseIntOrZero(String.valueOf(txtMatchesPerTeam.getText()));
                if (!MatchScheduler.isValidChampions(nTeams, k)) return 0;
                return MatchScheduler.nMatchesChampions(nTeams, k);
            }
            default:
                return MatchScheduler.nMatchesItalianBracket(tournamentKey) * (homeAndAway ? 2 : 1);
        }
    }
    ...
    private void updateInfo() {
        ...
        int capacity = Times.capacityMatches(readSlots(), minutes);
    ...
    private void confirm() {
        ...
        switch (format) {
            case GROUPS: {
                ...
                } else if (Times.checkInputTime(slots, minutes,
                        MatchScheduler.nMatchesMultipleBracket(tournamentKey, nBrackets) * (homeAndAway ? 2 : 1))) {
            ...
            case CHAMPIONS: {
                int k = parseIntOrZero(String.valueOf(txtMatchesPerTeam.getText()));
                if (!MatchScheduler.isValidChampions(nTeams, k)) {
                    Toast.makeText(this, R.string.invalid_champions_config, Toast.LENGTH_SHORT).show();
                } else if (Times.checkInputTime(slots, minutes, MatchScheduler.nMatchesChampions(nTeams, k))) {
            ...
            default: {
                if (Times.checkInputTime(slots, minutes,
                        MatchScheduler.nMatchesItalianBracket(tournamentKey) * (homeAndAway ? 2 : 1))) {
    ...
    private void createMultipleBracket(ArrayList<String> stringsInput, String txtMinutesForMatch, int nBrackets, boolean homeAndAway) {
        ...
        MatchScheduler.generateMultipleBracket(tournamentKey, stringsInput, txtMinutesForMatch, nBrackets, homeAndAway);
        finish();
    }

    private void createItalianBracket(ArrayList<String> stringsInput, String txtMinutesForMatch, boolean homeAndAway) {
        Toast.makeText(this, R.string.generating, Toast.LENGTH_LONG).show();
        MatchScheduler.generateItalianBracket(tournamentKey, stringsInput, txtMinutesForMatch, homeAndAway);
        finish();
    }

    private void createChampions(ArrayList<String> stringsInput, String txtMinutesForMatch, int k) {
        Toast.makeText(this, R.string.generating, Toast.LENGTH_LONG).show();
        MatchScheduler.generateChampions(tournamentKey, stringsInput, txtMinutesForMatch, k);
        finish();
    }
