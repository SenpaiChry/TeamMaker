...
import com.teammaker.app.ui.adapter.TournamentSpinnerTeamAdapter;
...
public class EditMatchActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (tournament.nBracket == 1) {
            groupCodes.add(MatchPhases.GROUP);
        } else if (tournament.nBracket > 1) {
            for (int i = 0; i < tournament.nBracket; i ++) {
                groupCodes.add(MatchPhases.groupCode(MatchScheduler.getBracketLetter(i)));
            }
        }
        for (Match m : tournament.matches) {
            if (MatchPhases.isGroup(m.type)) {
        ...
        for (String code : groupCodes) {
            if (code.equals(MatchPhases.GROUP)) {
                stringPhases.add(getString(R.string.bracketSpace).trim());
            } else {
                stringPhases.add(getString(R.string.bracket) + " " + code.substring(MatchPhases.GROUP_PREFIX.length()));
            }
            phaseCodes.add(code);
        }
        ...
        if (position < 0) { // NEW
            ...
            btnConfirm.setOnClickListener(view -> {
                if (Times.isValidTime(String.valueOf(txtTime.getText()))
                    ...
                    MatchRepository.addNewMatch(tournamentKey, match);
                    finish();
                }
            });
        } else { // EDIT
    ...
    private int getIndexByPhase(String phase) {
        ...
        String target = MatchPhases.normalize(phase);
